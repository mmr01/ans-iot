...
do
    echo "    Test" $counter.
    /opt/mssql-tools/bin/sqlcmd -U powerdb -S . -v varNumberOfPacketsPerHour=$1 -i ~/art-ans/src/t-sql/DB-90_Select_tests_SQLCMD.sql >/dev/null 
    echo "    DB-90_Select_tests_SQLCMD.sql - executed."
    echo "    Sleeeping for 30 seconds."
    sleep 30
    counter=$((counter+1)) ;
done
echo
