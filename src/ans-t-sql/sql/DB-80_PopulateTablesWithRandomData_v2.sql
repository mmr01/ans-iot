...
NOTE:               The number of packets sent from sensors to the server per hour
                    should be put to @numberOfPacketsPerHour variable below
					                 ^^^^^^^^^^^^^^^^^^^^^^^
					                 !!!!!!!!!!!!!!!!!!!!!!!
...
TRUNCATE TABLE t_readouts_source;
GO

TRUNCATE TABLE t_readouts_raw;
GO

TRUNCATE TABLE t_readouts_ans_tsql;
GO

TRUNCATE TABLE t_readouts_ans_cs;
GO

TRUNCATE TABLE t_readouts_raw_ans;
GO

TRUNCATE TABLE t_readouts_compress;
GO
...
DECLARE @numberOfPacketsPerHour TINYINT = 1
...
SELECT 1, 'SensorRaw -> SQLServer',60, 
qs.[sql_handle], [text], 
...
--Display metrics:
INSERT INTO dbo.metrics_insert 
SELECT 2, 'SensorRaw -> SQLServer -> COMPRESS',60, 
qs.[sql_handle], [text], 
execution_count, last_execution_time, 
total_worker_time/1000 AS total_worker_time_ms,
total_worker_time/execution_count/1000 AS avg_worker_time_ms,
...
DECLARE @numberOfTemperaturesInOneRow INT = 60;
DECLARE @t1 DATETIME;
DECLARE @t2 DATETIME;
DECLARE @elapsedTime_ms INT = 0;
SET @t1 = GETDATE();
INSERT INTO t_readouts_ans_tsql(
    ...
    SELECT rea_id, rea_sen_no, rea_audit_cd, 
	--rea_sequence,
...
--Display metrics:
INSERT INTO dbo.metrics_insert 
SELECT 3, 'SensorRaw -> SQLServer -> ANS_TSQL',60, 
qs.[sql_handle], [text], 
execution_count, last_execution_time, 
total_worker_time/1000 AS total_worker_time_ms,
total_worker_time/execution_count/1000 AS avg_worker_time_ms,
...
--Display metrics:
INSERT INTO dbo.metrics_insert 
SELECT 4, 'SensorRaw -> SQLServer -> ANS_CS',60, 
qs.[sql_handle], [text], 
execution_count, last_execution_time, 
total_worker_time/1000 AS total_worker_time_ms,
total_worker_time/execution_count/1000 AS avg_worker_time_ms,
...
--Display metrics:
INSERT INTO dbo.metrics_insert 
SELECT 5, 'SensorANS -> SQLServer',60, 
qs.[sql_handle], [text], 
execution_count, last_execution_time, 
total_worker_time/1000 AS total_worker_time_ms,
total_worker_time/execution_count/1000 AS avg_worker_time_ms,
